...
namespace NcTejun.NcRead
{
	...
	/// 主型用ＮＣデータの変換。文字列の変換２
	/// </summary>
	class ConvOld_NcRun3 : Conv
	{
		private static Regex regG41 = new Regex("G4[12][A-Z;]");
		private static readonly Regex regG41IJK = new Regex("[IJK][-IJK0-9.]*");
		...
		public ConvOld_NcRun3(List<Output.NcOutput.NcToolL> toolList, NcdTool.Mcn1 mach)
		...
		public override OutLine ConvExec() {
			...
			if (txtd.B_g100) {
				...
				if (regUVW.Match(txtd.NcLine).Success || regUKW.Match(txtd.NcLine).Success)
					LogOut.CheckCount("ConvOld_NcRun3 146", false, "G100 UVW,UKW Exists");
				switch (mach.ID) {
			...
			case Machine.MachID.KENSYO:
				...
				if (txtd.B_g100) {
					if (txtd.NcLine.IndexOf('B') >= 0)
						txtd.OutLine.Set(Substitute(txtd.OutLine.Txt,
							new Regex("B[0-9]*"),
							"B" + txtd.NumList.Find(num => num.ncChar == 'B').dblData.ToString("0.000"), false));
				}
				//9:
				//3:%,G100T,P-1:s:X0Y0::
		...
		private void sed_i(NcLineCode txtd) {

			//i:4:%,M0*3;,P-1:a:$r: クーラントコード追加
			if (ncQue.QueueMin <= -1 && regM03.Match(ncQue[-1].ncLine).Success) {
				int ll;
				switch (ncoutName.skog.tld.coolDB) {
				case "AIR": ll = mach.mcn2.cool[1]; break;
				case "OIL": ll = mach.mcn2.cool[2]; break;
				case "TAIR": ll = mach.mcn2.cool[3]; break;
				case "TOIL": ll = mach.mcn2.cool[4]; break;
				case "OFF": ll = 0; break;
				default: throw new Exception("qefdbqhedb");
				}
				if (ll != 0)
					txtd.outLine.atoAdd(String.Format("M{0:d}", ll));
			}
			//i:4:M0*5;:i:$R: クーラントオフコード追加
			if (regM05.Match(ncQue[0].ncLine).Success) {
				int ll;
				switch (ncoutName.skog.tld.coolDB) {
				case "OFF": ll = 0; break;
				default: ll = mach.mcn2.cool[0]; break;
				}
				if (ll != 0)
					txtd.outLine.maeAdd(String.Format("M{0:d}", ll));
			}
			return;
		}
		*/
		...
		private void OffsetD(NcLineCode txtd) {
			...
			if (regG41.Match(txtd.NcLine + ";").Success) {
				txtd.OutLine.Set(Substitute(txtd.OutLine.Txt, regG41IJK, "", false));
			}

			//3:G40M98P0*6[A-Z;(]:s:G40::
			if (regG40.Match(txtd.NcLine + ";").Success) {
				txtd.OutLine.Set(Substitute(txtd.OutLine.Txt, new Regex("G40"), "", false));
				LogOut.CheckCount("ConvOld_NcRun3 286", false, "G40M98P0006 Exists");
			}
		}
	}
}