...
namespace NcTejun.NcRead
{
	...
	class ConvOld_NcRun1 : Conv
	{
		...
		static private int Nctchk(ref int iget, string ddat) {
			...
			if (iget == 0) {
				if (ConvOld_NcRun1.tnget[0].Match(ddat).Success == false)
					return 0;
				iget++;
			}

			// 全行コメントの場合そのまま終了（次の行でチェック）
			if (ConvOld_NcRun1.tncmm.IsMatch(ddat)) return 0;

			if (ConvOld_NcRun1.tnget[iget].IsMatch(ddat) == false) {
				System.Windows.Forms.MessageBox.Show(
					$"ＮＣデータの開始{((int)(iget + 2)).ToString()}行目 \"{ddat}\" はGENERALのフォーマット \"{ConvOld_NcRun1.tnget[iget].ToString()}\" に適合していません。");
				// add 2010/05/21
				throw new Exception(
					$"ＮＣデータの開始{((int)(iget + 2)).ToString()}行目 \"{ddat}\" はGENERALのフォーマット \"{ConvOld_NcRun1.tnget[iget].ToString()}\" に適合していません。");
			}
		...
		public ConvOld_NcRun1(List<Output.NcOutput.NcToolL> toolList, NcdTool.Mcn1 mach)
		...
		public override OutLine ConvExec() {
			...
			ConvOld_NcRun1.Nctchk(ref iget, txtd.NcLine + ";");
			...
			if (ncname.Nmgt.Dimn == '3') {
				if (mach.ncCode.HSP_ON != null) {
					if (mach.ncCode.HSP_OFF == "G91X0Y0Z0")
						...
						LogOut.CheckCount("ConvOld_NcRun1 117", false, "不要であるが旧のルーチンとの整合を取るために挿入する暫定処置");
						Sed_g(txtd, mach.ncCode.HSP_ON, "G91X0Y0Z0", mach.ncCode.HSP_OFF);		// g
					}
				}
				else if (mach.ID == Machine.MachID.V77) {
					LogOut.CheckCount("ConvOld_NcRun1 123", false, "不要であるが旧のルーチンとの整合を取るために挿入する暫定処置");
			...
			if (regG39.Match(txtd.NcLine).Success) {
				LogOut.CheckCount("ConvOld_NcRun1 154", false, "G39 Exists");
				txtd.OutLine.Set("");
			}
			...
			default:
				if (txtd.NcLine.IndexOf(ClLink5Axis.Kotei) >= 0)
					LogOut.CheckCount("ConvOld_NcRun1 164", false, "工程間移動あり " + mach.name);
				break;
			}
			if (ClLink5Axis.KOTEI_IDO(mach.ID) == false) {
				//1:G65P9699:E:
				if (txtd.NcLine.IndexOf(ClLink5Axis.Kotei) >= 0)
					throw new Exception(mach.name + "は工程間移動のＮＣデータには対応していません in ConvOld");
			}
			// /////////
			// EDITEND
			// /////////
		...
		private void Sed_a(NcLineCode txtd) {
			...
			if (txtd.B_g100) {
				...
				ddat = Substitute(ddat, new Regex("T[0-9]*"), "T" + NcoutName.Skog.MatchK0.K2.Tnum.ToString("00"), false);
				ddat = Substitute(ddat, new Regex("S[0-9]*"), "S" + NcoutName.Skog.CutSpinRate().ToString("00000"), false);
			}

			// c:1:G00G90X,G100T,P-1:s:X.*:$x;:
			if (this.NcQue[0].NcLine.IndexOf("G00G90X") >= 0 && this.NcQue[-1].B_g100) {
				ddat = Substitute(ddat, new Regex("X.*"),
					"X" + ((double)ncname.Ncdata.ncInfo.xmlD.OriginX * 1000.0).ToString("f0") +
					"Y" + ((double)ncname.Ncdata.ncInfo.xmlD.OriginY * 1000.0).ToString("f0"), false);
			}
			...
			*/
			...
			if (regG41G66.Match(txtd.NcLine).Success)
				ddat = Substitute(ddat, new Regex("D[0-9]*"), "D" + ((int)NcoutName.Skog.MatchK0.K2.Tnum + mach.ncCode.DIN).ToString("00"), false);
		...
		private void Sed_h(NcLineCode txtd) {
			...
			if (regG41.Match(ddat).Success)
				ddat = Substitute(ddat, new Regex("[IJKD]-*[0-9.]*"), "", false);
		...
		private void Sed_g(NcLineCode txtd, string on, string off1, string off2) {
			...
			if (this.NcQue.QueueMin <= -3) {
				...
				if (this.NcQue[-3].NcLine.IndexOf(ClLink5Axis.Kotei) >= 0) {
					LogOut.CheckCount("ConvOld_NcRun1 306", false, this.ncname.Nnam);
					if (on != null) txtd.OutLine.MaeAdd(on);
				}

				/*
				// g:1:%,G65P9699,P-3:i:G05;:
				if (this.ncQue[-3].ncLine.IndexOf("G65P9699") >= 0) {
					LogOut.CheckCount("ConvOld_NcRun1 257", this.ncname.nnam);
					if (on != null) txtd.outLine.maeAdd(on);
				}
				*/
			}
