...
namespace NcTejun.NcRead
{
	...
	/// 主型用ＮＣデータの変換。マクロ展開
	/// </summary>
	class Conv_NcRun2 : Conv, IDisposable
	{
		...
		public Conv_NcRun2(List<Output.NcOutput.NcToolL> toolList, NcdTool.Mcn1 mach, _main_nctnk p_nctnk)
		...
		public override OutLine ConvExec() {
			...
			if (txtd.B_g6 && txtd.G6 != 67) {
				...
				if (mac_name == "P8400") {
					CamUtil.LogOut.CheckCount("Conv_NcRun2 042", false, NcdTool.Tejun.TejunName + " P8400マクロが未登録の加工機でのリジッドタップ加工にはまだ対応できていません。");
					throw new Exception("P8400マクロが未登録の加工機でのリジッドタップ加工にはまだ対応できていません。");
				}
				// チェック出力
				CamUtil.LogOut.CheckOutput(CamUtil.LogOut.FNAM.MACROTENK, NcdTool.Tejun.TejunName, NcdTool.Tejun.Mach.name,
					$"tncname={this.ncname.Nnam,-10} macro={txtd.NcLine}");
