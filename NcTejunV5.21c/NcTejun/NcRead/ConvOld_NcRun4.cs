...
namespace NcTejun.NcRead
{
	...
	class ConvOld_NcRun4 : Conv
	{
		...
		public ConvOld_NcRun4(List<Output.NcOutput.NcToolL> toolList, NcdTool.Mcn1 mach)
		...
		public override OutLine ConvExec() {
			...
			if (ncname.Nggt.Mirr != NcZahyo.Null)
				LogOut.CheckCount("ConvOld_NcRun4 066", false, "ミラーＸＹ実行");
			if (trns != Vector3.v0)
				LogOut.CheckCount("ConvOld_NcRun4 068", false, "移動実行");
			// ミラー移動後の座標値設定
			...
			// 固定サイクル・カスタムマクロ
				...
				if (ncname.Nggt.Mirr != NcZahyo.Null || trns != Vector3.v0) {
					...
					case CamUtil.CamNcD.MacroCode.KAKO.K_CHK:
						...
						if (ProgVersion.Debug)
							LogOut.CheckCount("ConvOld_NcRun4 155", true, mCode.mess + "はミラー処理、移動処理のチェックがされていません。");
						else
				...
				if (inc)
					...
					case CamUtil.CamNcD.MacroCode.KAKO.K_NON:
						LogOut.CheckCount("ConvOld_NcRun4 170", true, mCode.mess + "がインクリメンタル指令で実行された。");
						break;
					}
