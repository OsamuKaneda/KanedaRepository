...
namespace NcTejun.NcRead
{
	...
	class Conv_NcRun5 : Conv
	{
		...
		public Conv_NcRun5(List<Output.NcOutput.NcToolL> toolList, NcdTool.Mcn1 mach)
		...
		private void HiSpeed(NcLineCode txtd, bool p9699) {
			...
			if (p9699) {
				...
				if (txtd.NcLine.IndexOf(ClLink5Axis.Kotei) >= 0) {
					LogOut.CheckCount("Conv_NcRun5 247", false, this.ncname.Nnam);
					txtd.OutLine.MaeAdd("G05P0");
				}
				//5:M98P9699:s:P9699:P8509:
				if (txtd.NcLine.IndexOf(ClLink5Axis.Kotei) >= 0) {
					LogOut.CheckCount("Conv_NcRun5 252", false, this.ncname.Nnam);
					txtd.OutLine.Set(txtd.OutLine.Txt.Replace(ClLink5Axis.Kotei, "M98P8509"));
				}
			}
			else {
				if (txtd.NcLine.IndexOf(ClLink5Axis.Kotei) >= 0)
					LogOut.CheckCount("Conv_NcRun5 260", false, this.ncname.Nnam);
			}
		}
