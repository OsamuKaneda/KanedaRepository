...
namespace NcCode.nccode
{
	...
	public readonly struct Gcode
	{
		...
		/// </param>
		internal Gcode(double value, List<double[]> yomikae, List<int[]> gCodeMacro) {
			/*
			if (Post.PostData['G'].sdgt < 0) throw new Exception("sdgtが設定されていない");
			*/
			// Ｇコードの読み替え
					...
					if (ival == ii[0]) {
						...
						if (0 == 0) {
							...
							for (int jj = 7050; jj <= 7059; jj++) {
								if (NcMachine.ParaValue(jj, 0) && ival == Math.Abs(NcMachine.ParaData(jj, 0))) {
									unmatch = false;
									if (gsw != 0) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
									if (macName != 1960 + jj) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
									if (macCode != ival) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
									if (NcMachine.ParaData(jj, 0) * unt0 < 0) {
										if (group != 12) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
										if ((NcMachine.ParaData(7000, 0) & 8) == 0) {
											CamUtil.LogOut.CheckZantei("Gcode 305", "gCodeMacroを使用するためのチェック");
											if (item != SetItem(66.0)) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
										}
										else {
											CamUtil.LogOut.CheckZantei("Gcode 309", "gCodeMacroを使用するためのチェック");
											if (item != SetItem(66.1)) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
										}
									}
									else {
										CamUtil.LogOut.CheckZantei("Gcode 314", "gCodeMacroを使用するためのチェック");
										if (group != 0) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
										if (item != SetItem(65.0)) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
									}
								}
							}
							if (unmatch) throw new Exception("gCodeMacroを使用するためのチェックでエラー");
						}
						return;
					}
				}
				if (0 == 0) {
					CamUtil.LogOut.CheckZantei("Gcode 325", "gCodeMacroを使用するためのチェック");
					for (int jj = 7050; jj <= 7059; jj++) {
						if (NcMachine.ParaValue(jj, 0) && ival == Math.Abs(NcMachine.ParaData(jj, 0)))
							throw new Exception("gCodeMacroを使用するためのチェックでエラー");
					}
				}
			...
			// エラーの処理
			if (gsw != 0) {
				string errs = value.ToString("00.0");
				switch (gsw) {
				case 3: _main.Error.Ncerr(3, "Gcode G" + errs + " "); break;
				case 2: _main.Error.Ncerr(1, "Gcode G" + errs + " "); break;
				case 1: _main.Error.Ncerr(1, "Gcode G" + errs + " is ignored.\n"); break;
				}
			}
		}
		...
		/// <param name="value">Ｇコード設定値</param>
		internal Gcode(double value)
		{
			/*
			if (Post.PostData['G'].sdgt < 0)
				throw new Exception("sdgtが設定されていない");
			*/
			item = SetItem(value);
			// Ｇコードグループ、Ｇコード処理区分の取得
			gsw = Gcode.Ncgset(value, out group);
		...
		public double ToDouble()
		{
			...
			*/
			if (item.HasValue == false)
				throw new Exception("Ｇコードが設定されていない");
			return (double)item.Value / unt0;
		}
		/// <summary>整数化の可否を判断します</summary>
		public bool ToInt_OK()
		{
			return item.Value == (int)(Math.Round(ToDouble()) * unt0);
		}
		/// <summary>整数値のＧコードを出力します。小数点付になる場合はエラーです</summary>
		public int ToInt()
		{
			double tmp = Math.Round(ToDouble());
			if ((int)Math.Round(tmp * unt0) != item.Value)
				throw new Exception("整数値のＧコード以外が呼び出された");
			return (int)tmp;
		}
		...
		public string ToStringAuto() {
			string form;
			string form2 = this.ToInt_OK() ? "00" : "00." + new string('0', (int)Math.Round(Math.Log10(unt0)));
			if (this.ToInt_OK())
				form = "00";
			else {
				switch (unt0) {
				case 1: form = "00"; break;
				case 10: form = "00.0"; break;
				case 100: form = "00.00"; break;
				case 1000: form = "00.000"; break;
				case 10000: form = "00.0000"; break;
				default: throw new Exception("arhgbajerhfbahfr");
				}
				CamUtil.LogOut.CheckZantei("Gcode 467", "新しいform2のチェック");
			}
			if (form != form2) throw new Exception("新しいform2のチェックでエラー");
